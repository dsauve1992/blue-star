        ...
        <div className="flex items-center space-x-8">
          ...
          <ul className="hidden md:flex items-center space-x-1">
            {navItems.map((item, index) => (
              ...
              >
                <Link
                  to={item.navigationPath ?? item.path}
                  className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${location.pathname === item.path || (item.path !== '/' && location.pathname.startsWith(item.path.replace(/:.+/, '')))
