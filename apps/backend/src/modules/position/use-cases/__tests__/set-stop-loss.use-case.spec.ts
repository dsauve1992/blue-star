...
describe('SetStopLossUseCase', () => {
  ...
  beforeEach(async () => {
    ...
    useCase = module.get<SetStopLossUseCase>(SetStopLossUseCase);
  });

  describe('Happy Path', () => {
    it("should set stop loss for user's position", async () => {
  ...
  describe('Authorization', () => {
    it("should throw error when user tries to set stop loss on another user's position", async () => {
  ...
  describe('Edge Cases', () => {
    it('should handle position without note', async () => {
      const userId = UserId.of('user-123');
      const positionId = PositionId.of('position-123');
      const existingPosition = Position.fromEvents(positionId, userId, [
        {
