...
@Injectable()
export class ThemeCronService implements OnModuleInit {
  ...
  async onModuleInit() {
    this.logger.log('Running initial theme extraction on startup...');
    try {
      await this.themeService.extractAndSaveThemes();
      this.logger.log('Initial theme extraction completed successfully');
    } catch (error) {
      this.logger.error(
        `Initial theme extraction failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
